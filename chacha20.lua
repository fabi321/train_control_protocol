...
local app, concat, fff = table.insert, table.concat, 0xffffffff

------------------------------------------------------------

-- chacha quarter round (rotl inlined)
function qround(st,x,y,z,w)
	...
	a = (a + b) & fff
	--d = rotl32(d ~ a, 16)
	t = d ~ a ; d = ((t << 16) | (t >> (16))) & fff
	c = (c + d) & fff
	--b = rotl32(b ~ c, 12)
	t = b ~ c ; b = ((t << 12) | (t >> (20))) & fff
	a = (a + b) & fff
	--d = rotl32(d ~ a, 8)
	t = d ~ a ; d = ((t << 8) | (t >> (24))) & fff
	c = (c + d) & fff
	--b = rotl32(b ~ c, 7)
	t = b ~ c ; b = ((t << 7) | (t >> (25))) & fff
...
local chacha20_block = function(key, counter, nonce)
	...
	for i = 1, 16 do st[i] = (st[i] + wst[i]) & fff end
...
function chacha20(key, counter, nonce, pt)
